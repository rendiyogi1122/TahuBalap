...
@section('subtitle', 'Ubah informasi kategori produk')
      ...
      <form action="{{ route('admin.kategori.update', $kategori) }}" method="POST" class="space-y-6">
        ...
        <div>
          ...
          <textarea id="deskripsi" name="deskripsi" rows="5"
            ...
            placeholder="Jelaskan jenis dan karakteristik produk dalam kategori ini...">{{ old('deskripsi', $kategori->deskripsi) }}</textarea>
          @error('deskripsi')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
          @enderror
        </div>
